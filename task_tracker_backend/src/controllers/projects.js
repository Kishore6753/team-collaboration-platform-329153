...
class ProjectsController {
  ...
  async list(req, res, next) {
    try {
      ...
      const result = await query(
        `SELECT DISTINCT p.id, p.name, p.description, p.owner_user_id, p.created_at
         FROM projects p
  ...
  /**
   * PUBLIC_INTERFACE
   * Create a project owned by current user and add them as an admin member.
   */
  ...
  /**
   * PUBLIC_INTERFACE
   * Add a member to a project (owner/admin only in a future iteration; currently owner only).
   */
